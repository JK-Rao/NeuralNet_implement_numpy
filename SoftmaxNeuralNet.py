...
def show_ans(x_s, y_s, m_s, subplt):
    for k in range(0, m_s):
        if y_s[0, k] == 1:
            ...
            subplt.plot(x_s[0, k], x_s[1, k], 'yo')
        elif y_s[3, k] == 1:
            subplt.plot(x_s[0, k], x_s[1, k], 'co')
    # y1 = -b_s / w_s[0, 1]
...
def show_class(W, b, n, g, gamma, max, min, subplt):
    ...
    for i in range(len(n)):
        ...
        if g[i] == 'RELU':
            ...
            A_t.append(np.exp(Z_cali_t[i]) / np.sum(np.exp(Z_cali_t[i]), 0))
    Y_tra_t = cp.deepcopy(A_t[len(n) - 1])
    for k in range(Y_tra_t.shape[1]):
        index = np.where(Y_tra_t[:, k] == np.max(Y_tra_t[:, k]))[0][0]
        Y_tra_t[:, k] = 0
        Y_tra_t[index, k] = 1
...
for j in range(loop):
    ...
    for i in range(len(n)):
        ...
        if g[i] == 'RELU':
            ...
            A.append(np.exp(Z_cali[i]) / np.sum(np.exp(Z_cali[i]), 0))
    Y_tra = cp.deepcopy(A[len(n) - 1])
    Y_tra[Y_tra >= 0.5] = 1
    Y_tra[Y_tra < 0.5] = 0
    print("-------------------------")
    if g[len(n) - 1] == 'sigmoid':
        J = -(np.dot(Y, np.log(A[len(n) - 1]).T) + np.dot((1 - Y), np.log(1 - A[len(n) - 1]).T))[0][0] / m
    elif g[len(n) - 1] == 'softmax':
        J = -np.sum(Y * np.log(A[len(n) - 1])) / m
    print(J)
    if J < 0.01:
        break
    err[0].append(j + 1)
    ...
    for i in range(len(n)):
        ...
        dJdsigmd[len(n) - 1 - i] = np.sum(
            dJdZ_norm[len(n) - 1 - i] * (Z[len(n) - 1 - i] - miu[len(n) - 1 - i]) * (-0.5) * np.power(
                (sigmd[len(n) - 1 - i] + epsilon), -1.5), 1, None, None, True)
        dJdmiu[len(n) - 1 - i] = np.sum(-dJdZ_norm[len(n) - 1 - i] / np.sqrt(sigmd[len(n) - 1 - i] + epsilon), 1, None,
                                        None, True)
        dJdZ[len(n) - 1 - i] = dJdZ_norm[len(n) - 1 - i] / np.sqrt(sigmd[len(n) - 1 - i] + epsilon) + dJdsigmd[
            len(n) - 1 - i] * 2 * (Z[len(n) - 1 - i] - miu[len(n) - 1 - i]) / m + dJdmiu[len(n) - 1 - i] / m
